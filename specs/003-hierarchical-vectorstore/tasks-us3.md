...
- [ ] [T300] [US3] Create `src/tools/vectorstore/context-generator.ts` with `ContextConfig` interface: `context_max_tokens` (50-200, default 100), `context_concurrency` (1-50, default 10), `batch_size` (number of chunks per Claude request)
...
- [ ] [T303] [US3] Implement `contextualizeChunkBatch(document: string, batch: DocumentChunk[], config: ContextConfig): Promise<Map<string, string>>` in `src/tools/vectorstore/context-generator.ts` — calls Claude Agent SDK `query()` with model `claude-haiku-4-5`, `permissionMode: "acceptAll"`, `maxTurns: 1`, `allowedTools: []`; parses streaming response; validates with Zod schema; returns map of chunk_id to context string
- [ ] [T304] [US3] Implement retry logic with exponential backoff in `contextualizeChunkBatch` — on rate limit or error subtype, retry up to 3 times with delays of 1s, 2s, 4s; on persistent failure, reduce concurrency by half and retry
...
- [ ] [T307] [US3] Add graceful degradation in `generateContexts` — if a batch fails after all retries, log warning via structured logger (`src/lib/logger.ts`, per Constitution Principle VII) and return empty context for those chunks (embed without context rather than failing entire pipeline)

## Integration with Ingestion Pipeline

- [ ] [T308] [US3] Add `contextualized_content` optional field to `DocumentChunk` interface in `src/tools/vectorstore/types.ts` — stores context + original content for embedding
- [ ] [T309] [US3] Wire context generation into `src/tools/vectorstore/index.ts` ingestion flow — after chunking and before embedding: if `contextual_embeddings: true`, call `generateContexts` with full document text and chunks
...
- [ ] [T313] [US3] Map YAML `contextual_embeddings`, `context_max_tokens`, and `context_concurrency` fields from `HierarchicalDocumentToolSchema` to `ContextConfig` in `src/tools/vectorstore/index.ts`
...
- [ ] [T319] [P] [US3] Unit test retry and backoff in `tests/unit/tools/vectorstore/context-generator.test.ts` — mock query to fail twice then succeed, verify 3 attempts made with increasing delays
